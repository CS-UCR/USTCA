...
directory = 'outputs/region_observations'
...
cursor.execute("SELECT * FROM region_observations")
...
temp_df = spark_df.withColumn('date', to_date(spark_df['date'], 'yyyy-MM-dd'))
...
anovaMax = smf.ols('rate_of_change ~ region', data=tmax_region_pd).fit()
anovaMax_summary = anovaMax.summary()

anovaMin = smf.ols('rate_of_change ~ region', data=tmin_region_pd).fit()
...
plt.tight_layout()
plt.savefig(os.path.join(directory, 'outputs/region_analysis/residuals_distribution.png'))
plt.close()

plt.figure()
stats.probplot(resid_max, dist='norm', plot=plt)
plt.title('Q-Q Plot for TMAX Residuals')
plt.savefig(os.path.join(directory, 'outputs/region_analysis/qq_plot_tmax.png'))
plt.close()

plt.figure()
stats.probplot(resid_min, dist='norm', plot=plt)
plt.title('Q-Q Plot for TMIN Residuals')
plt.savefig(os.path.join(directory, 'outputs/region_analysis/qq_plot_tmin.png'))
plt.close()
...
plt.tight_layout()
plt.savefig(os.path.join(directory, 'outputs/region_analysis/residuals_vs_fitted.png'))
plt.close()
...
plt.tight_layout()
plt.savefig(os.path.join(directory, 'outputs/region_analysis/scatterplot_tmin_region.png'))
plt.close()
...
plt.tight_layout()
plt.savefig(os.path.join(directory, 'outputs/region_analysis/scatterplot_tmax_region.png'))
plt.close()
